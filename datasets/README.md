...
本目录只保存下载工具、固定版本和校验清单，**不保存 MELD、EmotionTalk 或其他第三方数据的原始文本、标签、音频、视频、压缩包和派生特征**。

这样做有三个目的：

1. 每位协作者从官方入口取得数据并独立接受许可证或 gated 条款；
2. 原始数据不经过公开 GitHub 二次托管；
3. MELD/EmotionTalk 的 test 角色继续由指定 evaluator 封存，避免开发阶段读取标签。
...
- 不得提交：原始 archive、解压文件、标签、媒体、逐样本索引、派生特征、权重和私有绝对路径；
- 传递数据时只发送官方链接和校验清单，不通过 GitHub、聊天附件或网盘二次分发；
- test 数据访问者与模型开发者分离，最终只返回 aggregate-only 评估结果。
