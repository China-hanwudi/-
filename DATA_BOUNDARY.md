...
- 自行编写的实验代码、合同测试和冻结配置；
- 只指向官方来源的下载脚本、固定 revision、字节数和 SHA-256 校验清单；
...
研究者必须从数据集官方入口自行申请或下载，并独立遵守许可、署名、使用和再分发条件。协作者使用 [`datasets/`](datasets/) 中的固定版本、下载工具和校验清单建立本地数据目录；下载物不得重新复制回本仓库。

MELD 默认只获取 train/dev 标注，test 仅由指定 evaluator 获取。EmotionTalk 的官方 archive 会同时覆盖多个数据角色；即使协作者已合法下载，开发人员也不得读取或提取 test 标签。最终 evaluator 只返回 aggregate-only 结果。

仓库的 MIT License 只覆盖本仓库自行编写的代码和文档，不改变第三方数据、模型或媒体的许可证。
