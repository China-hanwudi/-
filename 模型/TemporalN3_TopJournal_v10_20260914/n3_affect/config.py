...
DEFAULT_LABELS = (
    # Official M3ED class order, verbatim from M3ED/packed/data_audit.json
    # (label_field = EmoAnnotation.final_main_emo).  The previous tuple
    # (anger, disgust, fear, joy, neutral, sadness, surprise) was an
    # alphabetised guess that did not match the packed label ids.
    "Happy",
    "Neutral",
    "Sad",
    "Disgust",
...
class N3TrainConfig:
    ...
    # Drop current modalities during training as a missing-modality
    # augmentation.  At least one observed stream is always retained.
    current_modality_dropout: float = 0.10
    ...
    # Label-anchor every observed current stream against the task label.  This
    # is available on M3ED even though it has no modality-specific labels.
    current_unimodal_loss_weight: float = 0.12
    ...
    # ---- ablation switches (evaluation protocol; never on for the mainline) --
    # ``disable_history`` zeroes every history slot before any downstream path,
    # so train and eval are exactly current-only ("drop history" row of Table 4).
    disable_history: bool = False
    # ``disable_relation`` removes the candidate-level shared 3x3 current/history
    # evidence grid; downstream tensors keep their shapes with zero evidence
    # ("drop candidate-level 3x3" row of Table 4).
    disable_relation: bool = False
    # ``none`` | ``sqrt_inv`` | ``inv``: class weighting for the primary cross
    # entropy.  Kept off for the mainline result; offered because M3ED's rarest
    # class (Fear) is only 1.6% of train and Macro-F1 is a co-primary metric.
    ...
    def validate(self) -> None:
        ...
        if any(weight < 0.0 for weight in (
            ...
            self.current_unimodal_loss_weight,
        )):
        ...
        if any(not 0.0 <= p < 1.0 for p in (
            self.history_dropout, self.modality_dropout, self.current_modality_dropout
        )):
            raise ValueError("history/modality dropout must be in [0, 1)")
        if self.class_weight_mode not in {"none", "sqrt_inv", "inv"}:
            raise ValueError("class_weight_mode must be none, sqrt_inv or inv")
