...
def n3_regression_loss(outputs: dict, targets: Tensor, cfg: N3RegressionConfig) -> dict[str, Tensor]:
    ...
    # Huber keeps the stable MAE-like objective used for checkpoint selection,
    # while its quadratic basin supplies useful gradients for the long-tail
    # sentiment errors visible in MOSEI.  The small L1 term preserves the
    # calibration of the original contract and avoids chasing outliers.
    residual = outputs["prediction_norm"] - normalized
    exact_l1 = residual.abs().mean()
    primary = 0.75 * F.smooth_l1_loss(
        outputs["prediction_norm"], normalized, beta=0.20
    ) + 0.25 * exact_l1
    zero = primary.new_zeros(())
    current_aux = history_aux = risk = coverage = safe_coverage = risk_positive_rate = zero
    total = primary
    current_uni_loss = zero
    current_uni_w = float(getattr(cfg, "global_unimodal_loss_weight", 0.0) or 0.0)
    current_uni = outputs.get("current_unimodal_prediction_norm")
    current_mask = outputs.get("modality_mask")
    if current_uni is not None and current_mask is not None and current_uni_w > 0:
        target = normalized.expand_as(current_uni)
        mask = current_mask.to(dtype=current_uni.dtype)
        current_uni_loss = (F.smooth_l1_loss(current_uni, target, beta=0.20, reduction="none") * mask).sum() / mask.sum().clamp_min(1.0)
        total = total + current_uni_w * current_uni_loss
    ...
    return {"loss": total, "regression_objective": primary,
            "main_l1_normalized": exact_l1, "mae_original_scale": exact_l1 * cfg.target_scale,
            "current_aux_loss": current_aux, "history_aux_loss": history_aux, "risk_loss": risk,
            "current_unimodal_loss": current_uni_loss,
