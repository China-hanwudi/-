...
# The scalar-regression backbone is dataset-agnostic; only the label scale and
# the binarisation convention differ.  Both active regression datasets are
# listed explicitly so an unknown task string still fails loudly.
ALLOWED_REGRESSION_TASKS = frozenset({
    "mosei_sentiment_regression",
    "chsims_v2_sentiment_regression",
})
...
class N3RegressionConfig:
    ...
    # Supervise each observed current stream against the fused task label on
    # datasets without modality-specific annotations (notably MOSEI).
    global_unimodal_loss_weight: float = 0.08
    ...
    def validate(self) -> None:
        ...
        if self.task not in ALLOWED_REGRESSION_TASKS or self.output_dim != 1:
            raise ValueError(
                f"This model implements scalar sentiment regression only; got task={self.task!r} "
                f"(allowed: {sorted(ALLOWED_REGRESSION_TASKS)})")
        ...
        if any(not 0 <= p < 1 for p in (
            self.dropout, self.history_dropout, self.modality_dropout,
            self.current_modality_dropout,
        )):
        ...
        if any(w < 0 for w in (self.current_aux_loss_weight, self.history_aux_loss_weight,
                              ...
                              self.global_unimodal_loss_weight)):
