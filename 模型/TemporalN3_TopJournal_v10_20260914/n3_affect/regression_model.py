...
class N3SentimentModel(nn.Module):
    def __init__(self, cfg: N3RegressionConfig) -> None:
        ...
        self.current_unimodal_head = UnimodalContributionHead(d, cfg.gate_hidden, 1, cfg.dropout)
        self.current_modality_gate = nn.Sequential(
            nn.Linear(d, cfg.gate_hidden), nn.GELU(), nn.Dropout(cfg.dropout),
    ...
    def _current_prediction(self, tokens: Tensor, modality_mask: Tensor) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        pooled = self._encode_current(tokens, modality_mask)
        anchor = self.regressor(pooled)
        modal_pred = self.current_unimodal_head(tokens).squeeze(-1)
        gate_logits = self.current_modality_gate(tokens).squeeze(-1).masked_fill(~modality_mask.bool(), -1e4)
        weights = F.softmax(gate_logits, dim=-1) * modality_mask.to(gate_logits.dtype)
        weights = weights / weights.sum(dim=-1, keepdim=True).clamp_min(1e-6)
        anchored = (weights * modal_pred).sum(dim=1, keepdim=True)
        blend = torch.tanh(self.current_anchor_blend)
        return anchor + blend * (anchored - anchor), pooled, modal_pred, weights

    def _ablated_current_prediction(self, current_tokens: Tensor, modality_mask: Tensor,
                                    drop_modality: int) -> Tensor:
        ...
        return self._current_prediction(ablated, visible)[0]
    ...
    def measure_counterfactual_utility(self, batch: Mapping[str, Tensor], targets: Tensor) -> Tensor:
        ...
        try:
            with torch.no_grad():
                ...
                base = (self._current_prediction(tokens, modality_mask)[0] - targets).abs()
                collected = []
                for k in range(3):
                    removed = (self._ablated_current_prediction(tokens, modality_mask, k) - targets).abs()
    ...
    def forward(self, batch: Mapping[str, Tensor], route_mode: str = "hard-safe") -> dict:
        ...
        if bool(getattr(self.cfg, "disable_history", False)):
            # Ablation "drop history": the whole history branch becomes empty, so
            # hard-safe is exactly the current anchor by construction.
            history_mask = torch.zeros_like(history_mask)
            hist_modal_mask = torch.zeros_like(hist_modal_mask)
        ...
        if self.training and self.cfg.current_modality_dropout > 0:
            ...
            keep = torch.where((keep * modality_mask).sum(dim=1, keepdim=True) == 0,
                               torch.ones_like(keep), keep)
            modality_mask = modality_mask * keep
            current_tokens = current_tokens * modality_mask.unsqueeze(-1)
            for i, name in enumerate(("T", "A", "V")):
                streams[f"{name}_t"] = current_tokens[:, i]
        current, current_context, current_modal_prediction, current_modality_weights = self._current_prediction(
            current_tokens, modality_mask
        )
        ...
        if bool(getattr(self.cfg, "disable_relation", False)):
            # Ablation "drop candidate-level 3x3 relation module".
        ...
        return {
            ...
            "current_unimodal_prediction_norm": current_modal_prediction,
