...
def n3_total_loss(
    ...
    # Label-anchored current experts improve minority recall without using
    # validation/test statistics.  The same fused label supervises each
    # visible stream; this is an auxiliary term, never a causal difference.
    current_uni = outputs.get("current_unimodal_logits")
    current_mask = outputs.get("current_modality_mask")
    current_uni_loss = zero
    current_uni_w = float(getattr(cfg, "current_unimodal_loss_weight", 0.0) or 0.0)
    if current_uni is not None and current_mask is not None and current_uni_w > 0:
        mask = current_mask.bool()
        if mask.any():
            current_uni_loss = F.cross_entropy(
                current_uni[mask], labels[:, None].expand_as(mask)[mask],
                weight=class_weight, label_smoothing=smoothing,
            )
