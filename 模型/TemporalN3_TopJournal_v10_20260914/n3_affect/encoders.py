...
class ModalityProjector(nn.Module):
    """Normalize and adapt a pre-computed modality feature.

    The packed contracts provide frozen frame/utterance features rather than
    raw waveforms or pixels.  A plain linear projection was too brittle when
    one stream (notably MOSEI text/audio) had a different scale or noisy
    dimensions.  This adapter keeps the public tensor contract unchanged but
    adds an input normalization and a small residual bottleneck.  The
    residual is zero-initialised, so the base path is recovered at
    initialization. v9 checkpoints require an explicit migration because the
    adapter state dictionary has new normalization and bottleneck parameters.
    """
    def __init__(self, in_dim: int, d_model: int, dropout: float,
                 bottleneck: int = 32) -> None:
        super().__init__()
        ...
        width = max(8, min(int(bottleneck), d_model))
        self.residual_down = nn.Linear(d_model, width)
        self.residual_up = nn.Linear(width, d_model)
        nn.init.zeros_(self.residual_up.weight)
        nn.init.zeros_(self.residual_up.bias)
        self.output_norm = nn.LayerNorm(d_model)

    def forward(self, x: Tensor) -> Tensor:
        z = self.base(self.input_norm(x))
        residual = self.residual_up(F.gelu(self.residual_down(z)))
        return self.output_norm(z + residual)
