...
OUT="${OUT:-$ROOT/runs/v10_${DATASET}/seed_$(printf '%03d' "$SEED")}"
...
if [[ "${SMOKE:-1}" == "1" ]]; then
  "$PYTHON_BIN" -m n3_affect.train_m3ed --data "$DATA_ROOT/M3ED/packed" \
    --out "$ROOT/runs/v10_preflight_m3ed" --seed 17 --smoke 1 --device cuda
  test -f "$ROOT/runs/v10_preflight_m3ed/FINAL_RESULT.json"
fi
