...
> **HISTORICAL DESIGN SOURCE / SUPERSEDED EXECUTION NOTE（2026-08-13）：**本文保留为 N3 设计和冻结协议的历史依据，不再是当前执行手册。编码器主线已由“文本/音频/视频分别使用情感专用模型”更新为“冻结 `Qwen3-Omni-30B-A3B-Instruct` 分别抽取 T/A/V，并保持当前/历史六路”；专用编码器改作 baseline/消融。当前固定分两阶段：Phase A 先做 emotion-only 管线基线（utility/VAD 权重为 0），Phase B 再启用完整 N3。Phase B 必须对每个 `h_k (k=1..3)` 分别计算 3×3，禁止先聚合历史；共享 3×3、模态/联合双向效用、两级门控和真实分类成功门继续保留。冲突处以 [2026-08-13 最新执行基线](14_最新执行基线与GitHub旧方案差异_2026-08-13.md) 为准。
...
按最新 Phase B 口径，上述九组关系不是对预先聚合的单个历史表示只算一次。必须对严格过去窗口中的 `h_1`、`h_2`、`h_3` 分别计算一组 3×3，并保留候选轴直到模态级/联合级效用与两级门控：

```text
R_k = 3×3(T_t, A_t, V_t ; h_k^T, h_k^A, h_k^V),  k=1,2,3
```

禁止先聚合 `h_1/h_2/h_3` 后再计算一次九组关系；该做法会丢失逐候选效用监督、模态部分保留和候选级回退所需的可归因性。
